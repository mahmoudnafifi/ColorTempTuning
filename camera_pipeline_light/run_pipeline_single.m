function [ output_image ] = run_pipeline_single( input_image, ...
    meta_info, input_stage, output_stage, cfastr,maxval  )
...
if strcmp(input_stage,'normal')
    ...
    wb_multipliers = meta_info.AsShotNeutral; % is this normalized? no
    wb_multipliers = wb_multipliers.^-1;
    mask = wbmask(size(lin_bayer,1),size(lin_bayer,2),wb_multipliers,cfastr);
    balanced_bayer = lin_bayer .* mask; % this is required
...
if strcmp(input_stage,'wb')
    ...
    if ~exist('maxval','var')
        maxval=max(balanced_bayer(:));  
    end
    temp = uint16(balanced_bayer/maxval*(2^16));
