...
function [xyz] = TempToXyz(temperature)
[xyz] = spectrum_to_xyz(@bb_spectrum, temperature);
end

function [emmitance] = bb_spectrum(wavelength, bbTemp)
    wlm = wavelength * 1e-9;   % Wavelength in meters
    emmitance = (3.74183e-16 * (wlm ^ -5.0)) / ...
           (exp(1.4388e-2 / (wlm * bbTemp)) - 1.0);
end

function [xyz] = spectrum_to_xyz(bb_spectrum_handle, temperature)
    cie_colour_match = {
    ...
    X = 0; Y = 0; Z = 0;
    i = 1;
    for lambda = 380 : 5: 780
        Me = bb_spectrum_handle(lambda, temperature);
        X = X + Me * cie_colour_match{1, i}{1};
        Y = Y + Me * cie_colour_match{1, i}{2};
        Z = Z + Me * cie_colour_match{1, i}{3};
        i = i + 1;
    end
    XYZ = (X + Y + Z);
    x = X / XYZ;
