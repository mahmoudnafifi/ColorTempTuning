...
if strcmp(input_stage,'normal')
    ...
    if ~ exist ('wb_raw','var') || strcmp(wb_raw, 'none') || isempty(wb_raw)
        ...
        % wb_multipliers = [1, 1, 1]; % no WB
    else
        wb_multipliers = wb_raw;
    end
        
    % wb_multipliers=wb_multipliers*(1/sqrt(3))
...
if strcmp(input_stage,'demosaic')
    ...
    if ~ exist ('xyz2cam','var') || strcmp(xyz2cam, 'none') || ...
            isempty(xyz2cam)
...
if strcmp(input_stage,'srgb')
    ...
    %size(tc);
    x = uint16(single(srgb) / 255.0 * (size(tc, 1) - 1) + 1);
    tone = tc(x) * 255.0;
